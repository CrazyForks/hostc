...
export async function createConnectToken(secret: string, subdomain: string): Promise<string> {
...
export async function verifyConnectToken(secret: string, subdomain: string, token: string): Promise<boolean> {
  ...
  if (payload.v !== CONNECT_TOKEN_VERSION || payload.subdomain !== subdomain || payload.exp < Date.now()) {
    return false;
  }
