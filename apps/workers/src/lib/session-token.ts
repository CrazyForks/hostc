...
export async function createSessionToken(secret: string, subdomain: string): Promise<string> {
...
export async function verifySessionToken(secret: string, subdomain: string, token: string): Promise<boolean> {
  ...
  if (payload.v !== SESSION_TOKEN_VERSION || payload.subdomain !== subdomain || payload.exp < Date.now()) {
    return false;
  }
